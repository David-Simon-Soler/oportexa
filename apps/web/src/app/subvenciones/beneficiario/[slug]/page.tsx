...
export async function generateMetadata({ params, searchParams }: Props): Promise<Metadata> { const entity = await getBeneficiaryTypeBySlug((await params).slug); const query = await searchParams; return entity ? { title: `Subvenciones para ${entity.label}`, description: `Convocatorias para ${entity.label} en los datos actualmente incorporados por Oportexa.`, alternates: { canonical: `/subvenciones/beneficiario/${entity.slug}` }, robots: entity.totalGrants > 0 ? noindexForQuery(query) : { index: false, follow: true } } : { robots: { index: false, follow: true } }; }
