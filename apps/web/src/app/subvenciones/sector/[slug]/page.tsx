...
export async function generateMetadata({ params, searchParams }: Props): Promise<Metadata> { const entity = await getSectorBySlug((await params).slug); const query = await searchParams; return entity ? { title: `Subvenciones de ${entity.label}`, description: `Convocatorias del sector ${entity.label} en los datos actualmente incorporados por Oportexa.`, alternates: { canonical: `/subvenciones/sector/${entity.slug}` }, robots: entity.totalGrants > 0 ? noindexForQuery(query) : { index: false, follow: true } } : { robots: { index: false, follow: true } }; }
