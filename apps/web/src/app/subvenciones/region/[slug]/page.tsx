...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> { const region = await getRegionBySlug((await params).slug); return region ? { title: `Subvenciones en ${region.label}`, description: `Convocatorias de ${region.label} en los datos actualmente incorporados por Oportexa.`, alternates: { canonical: `/subvenciones/region/${region.slug}` } } : {}; }
